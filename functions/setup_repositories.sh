...
setup_repositories() {
    ...
    DISTRO_NAME=$(grep VERSION_CODENAME /etc/os-release | cut -d '=' -f2)
    echo "deb [signed-by=/usr/share/keyrings/hashicorp-archive-keyring.gpg] https://apt.releases.hashicorp.com $DISTRO_NAME main" | tee /etc/apt/sources.list.d/hashicorp.list
    apt update -y
}
